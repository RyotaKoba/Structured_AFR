...
def eval_ppl(args, model, tokenizer, device=torch.device("cuda:0")):
    ...
    # Get the test loader
    _, testloader = get_loaders(seed=0, seqlen=model.seqlen, tokenizer=tokenizer)
    ...
    with torch.no_grad():
        ...
        ppl = eval_ppl_wikitext(args, model, testloader, 1, torch.device("cuda:0"))
    return ppl 
...
def show_model_input_output(model, tokenizer, device):
    _, testenc = get_loaders(seed=0, seqlen=model.seqlen, tokenizer=tokenizer)
    # モデルを評価モードに設定
    model.eval()
    ...
    for i in range(0, 5, 1):
        ...
        print("inputs:",tokenizer.decode(inputs[0], skip_special_tokens=True))
        # Forward pass through the model
        lm_logits = model(inputs).logits    

        idx = torch.argmax(lm_logits, dim=-1)
        print("_"*80)

        print("outputs:",tokenizer.decode(idx[0].tolist(), skip_special_tokens=True))
        break